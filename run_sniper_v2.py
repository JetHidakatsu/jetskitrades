...
async def run_sniper_strategy():
    """Run the memecoin sniper strategy for 3 trades"""
    ...
    try:
        ...
        for trade_num in range(1, 4):
            logger.info(f"\nExecuting trade {trade_num}/3")

            # Create sample pool metrics with slight variations
            pool_metrics = PoolMetrics(
                liquidity=0.1 + (trade_num * 0.02),  # Increasing liquidity
                volume_24h=0.05 + (trade_num * 0.01),  # Increasing volume
                price_impact=0.02 - (trade_num * 0.002),  # Decreasing price impact
                holder_count=10 + (trade_num * 5),  # Increasing holders
                creator_score=0.8,
                time_since_creation=1.0 + trade_num,  # Increasing age
                depth_scores={"depth1": 0.5 + (trade_num * 0.1)},  # Improving depth
                market_conditions=None,
                total_supply=1000000.0,
                quantum_score=0.85 + (trade_num * 0.05),  # Improving score
            )
            ...
            if result["status"] == "take_profit":
                logger.info(f"Trade {trade_num} - PROFIT: {result['profit']:.4f} SOL")
                logger.info(f"Moonbag retained: {result['moonbag']:.4f} SOL")
            elif result["status"] == "stop_loss":
                logger.info(f"Trade {trade_num} - LOSS: {result['loss']:.4f} SOL")
            else:
                logger.error(
                    f"Trade {trade_num} - ERROR: {result.get('error', 'Unknown error')}"
                )
        ...
        logger.info(f"Win Rate: {(len(moonbag_positions)):.1f}%")
