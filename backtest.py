...
class BacktestEngine:
    ...
    def __init__(
        ...
        helius_provider: Optional[HeliusDataProvider] = None,
    ):
        self.config = config
        ...
        self.logger.info(
            f"Initial capital: ${config.initial_capital:.2f} = {self.capital:.4f} SOL"
        )
    ...
    async def run_backtest(self, data_path: Optional[Path] = None) -> BacktestResults:
        ...
        try:
            ...
            if self.historical_data:
                ...
                self.logger.warning(
                    "No historical data found in the specified date range"
                )
    ...
    async def _load_helius_data(self) -> List[Dict]:
        ...
        try:
            if not self.helius_provider:
                load_dotenv()
                self.helius_provider = HeliusDataProvider(
                    api_key=os.getenv("HELIUS_API_KEY"),
                    endpoint=os.getenv("HELIUS_ENDPOINT", "https://api.helius.xyz/v0"),
                )

            # Get pool creation events
            pools = await self.helius_provider.get_pool_creations(
                self.config.start_date, self.config.end_date
            )

            # Check for empty pools after successful API call
            if not pools:
                raise Exception("No pools found in date range")
            ...
            for pool in pools:
                ...
                price_impacts = await self.helius_provider.get_price_impacts(
                    [pool.pool_id],
                        ...
                        historical_data.append(
                            {
                                "type": "price_update",
                                "timestamp": pool.timestamp
                                + timedelta(minutes=1),  # Approximate
                                "pool_id": pool.pool_id,
    ...
    def _initialize_trading_logic(self) -> TradingLogic:
        ...
        params = TradingParameters(
            ...
            min_liquidity=1000,
            max_total_exposure=0.3,  # 30% total exposure limit
            max_exposure_per_token=0.1,  # 10% per token limit
        )
        ...
        # Initialize mempool monitor with required dependencies
        mempool_monitor = MempoolMonitor(
            rpc_client, self.latency_tracker, pool_validator
        )
    ...
    async def _handle_pool_creation(self, event: Dict, trading_logic: TradingLogic):
        ...
        try:
            ...
            meets_criteria, score, analysis = (
                await trading_logic.evaluate_trading_opportunity(
                    pool_id, pool_data, pool_data.get("creator_address", "")
                )
            )

            self.logger.info(
                f"Evaluation results - Meets criteria: {meets_criteria}, Score: {score}"
            )
            ...
            if meets_criteria and len(self.positions) < self.config.max_positions:
                ...
                metrics = PoolMetrics(
                    ...
                    depth_scores={},  # Empty for now
                    market_conditions=None,  # None for now
                )

                # Score the pool using quantum selector
                quantum_score = self.quantum_selector.score_pool(metrics)
                metrics.quantum_score = quantum_score  # Update metrics with score

                # Calculate position size using the scored metrics
                size = (
                    await self.quantum_selector.get_optimal_size(metrics) * self.capital
                )

                self.logger.info(
                    f"Opening position - Size: {size}, Entry price: {pool_data['price']}"
                )
    ...
    async def _handle_price_update(self, event: Dict, trading_logic: TradingLogic):
        ...
        if pool_id in self.positions:
            ...
            # Score the pool using quantum selector
            quantum_score = self.quantum_selector.score_pool(pool_metrics)
            pool_metrics.quantum_score = quantum_score  # Update metrics with score

            # Check exit conditions
            should_exit = await trading_logic._check_exit_conditions(
                pool_id, current_price, pool_metrics, False  # Not a memecoin by default
            )
    ...
    async def _close_all_positions(self, timestamp: datetime):
        ...
        for pool_id in list(self.positions.keys()):
            final_price = last_prices.get(
                pool_id, self.positions[pool_id]["entry_price"]
            )
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["file", "helius"],
        default="file",
        help="Data source (file or Helius API)",
    )
    parser.add_argument(
    ...
    parser.add_argument("--risk", type=float, default=0.02, help="Risk factor")
