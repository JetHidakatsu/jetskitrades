...
class MarketSimulator:
    ...
    def generate_market_events(self, timestamp: datetime) -> List[Dict]:
        ...
        if random.random() < 0.1:  # 10% chance of market event
            event_type = random.choice(['market_volatility', 'network_congestion', 'sentiment_shift'])
            events.append({
                'timestamp': timestamp,
                'type': event_type,
...
class SimulationEngine:
    ...
    async def run_simulation(self):
        ...
        try:
            ...
            await self._close_all_positions(start_time + timedelta(hours=self.config.duration_hours))
    ...
    async def _close_position(self, pool_id: str, price: float, timestamp: datetime):
        ...
        try:
            ...
            self.logger.info(
                f"Closed position in pool {pool_id} "
    ...
    def _generate_simulation_report(self):
        ...
        success_rate = (
            self.metrics['successful_trades'] / total_trades 
...
def main():
    ...
    parser.add_argument('--pools', type=int, default=50,
