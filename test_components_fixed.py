...
async def test_distribution():
    ...
    # Initialize with version parameter
    client = AsyncClient(
    ...
    try:
        # Update distribution
        await analyzer.update_distribution(token_address)

        # Get summary
        summary = analyzer.get_distribution_summary(token_address)
...
async def test_liquidity():
    ...
    # Test pool (BONK/SOL)
    token_address = "DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263"
    pool_address = "8PhnCfgqpgFM7ZJvttGdBVMXHuU4Q23ACxCvWkbs1M71"

    try:
        # Update liquidity
        await analyzer.update_liquidity(token_address, pool_address)

        # Get summary
        summary = analyzer.get_liquidity_summary(token_address)
...
async def test_arbitrage():
    ...
    # Initialize WebSocket manager with URLs
    ws_urls = ["wss://api.mainnet-beta.solana.com", "wss://solana-api.projectserum.com"]
    ws_manager = WebSocketManager(urls=ws_urls)
...
async def main():
    ...
    # Test distribution analysis
    dist_success = await test_distribution()
    print(f"Distribution Analysis: {'✓' if dist_success else '✗'}")

    # Test liquidity analysis
    liq_success = await test_liquidity()
    print(f"Liquidity Analysis: {'✓' if liq_success else '✗'}")

    # Test arbitrage engine
    arb_success = await test_arbitrage()
