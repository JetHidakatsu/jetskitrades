...
from env.distribution_analyzer_v2 import DistributionAnalyzer, DistributionConfig
...
async def test_distribution(client):
    ...
    analyzer = DistributionAnalyzer(
        client, config, helius_api_key=None  # Add your Helius API key here
    )
    ...
    try:
        ...
        print("\nToken Metadata:")
        if summary["metadata"]:
            print(f"Supply: {summary['metadata']['supply']}")
            print(f"Decimals: {summary['metadata']['decimals']}")
            print(f"Authority: {summary['metadata']['authority']}")
            print(f"Freeze Authority: {summary['metadata']['freeze_authority']}")
        ...
        print(f"Whale Addresses: {len(summary['whale_addresses'])}")
