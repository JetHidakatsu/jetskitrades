...
def main():
    ...
    parser.add_argument(
        "--helius-only", action="store_true", help="Run only Helius integration tests"
    )
    parser.add_argument(
    ...
    pytest_args = ["-c", "env/tests/pytest_backtest.ini", "--verbose"]
    ...
    if args.html_report:
        report_dir = (
            Path("reports") / "test_results" / datetime.now().strftime("%Y%m%d_%H%M%S")
        )
        report_dir.mkdir(parents=True, exist_ok=True)
        pytest_args.extend(
            [f"--html={report_dir}/report.html", "--self-contained-html"]
        )
    ...
    # Add test timing
    pytest_args.extend(["--durations=10", "--durations-min=1.0"])
