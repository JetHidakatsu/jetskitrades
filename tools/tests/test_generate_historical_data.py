...
def test_data_validity(generator):
    ...
    for pool_id in generator.pools:
        pool_events = [
            e for e in events if e["pool_id"] == pool_id and e["type"] == "price_update"
        ]
        if pool_events:
...
def test_custom_parameters():
    ...
    assert all(
        ...
        for p in (
            e["pool_data"] for e in data["events"] if e["type"] == "pool_creation"
        )
    )
...
def test_error_handling():
    ...
    with pytest.raises(ValueError):
        HistoricalDataGenerator(
            start_date=datetime(2024, 1, 2), end_date=datetime(2024, 1, 1)
        )

    # Invalid pool count
    with pytest.raises(ValueError):
        HistoricalDataGenerator(
            start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 2), num_pools=0
        )
