        ...
        for i in range(self.num_pools):
            ...
            self.pools[pool_id] = {
                ...
                "creation_time": self.start_date + timedelta(
                    minutes=random.randint(0, int((self.end_date - self.start_date).total_seconds() / 60))
                )
            }
    ...
    def _create_dataset(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_initial_liquidity": sum(p["initial_liquidity"] for p in self.pools.values()) / len(self.pools),
                "avg_price_impact": sum(p["price_impact"] for p in self.pools.values()) / len(self.pools),
                "unique_creators": len(set(p["creator_address"] for p in self.pools.values()))
            },
            "scenarios": scenarios,
            "metrics": metrics
        }
    ...
    def _generate_scenarios(self) -> Dict[str, Any]:
        ...
        for pool_id, pool_data in self.pools.items():
            ...
            if len(pool_updates) > 1:
                ...
                duration = (
                    datetime.fromisoformat(pool_updates[-1]["timestamp"]) -
                ...
                scenario_type = (
                    "successful_trade" if profit_percentage > 10 else
                    "failed_trade" if profit_percentage < -10 else
                    "neutral_trade"
                )
                ...
                scenarios[scenario_type] = {
                    ...
                    "profit_percentage" if profit_percentage > 0 else "loss_percentage": abs(profit_percentage)
                }
...
def main():
    ...
    parser.add_argument('--num-pools', type=int, default=5,
                       help='Number of pools to generate')
    parser.add_argument('--update-frequency', type=int, default=60,
                       help='Update frequency in seconds')
    parser.add_argument('--base-liquidity', type=float, default=1000000,
                       help='Base liquidity amount')
