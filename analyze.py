...
class PerformanceAnalyzer:
    ...
    def _generate_detailed_report(self, output_dir: Path):
        ...
        # Time-based analysis
        hourly_stats = df.groupby(df["timestamp"].dt.hour)["profit"].agg(
            ["mean", "count"]
        )
        daily_stats = df.groupby(df["timestamp"].dt.day_name())["profit"].agg(
            ["mean", "count"]
        )
        ...
        # Duration analysis
        df["duration"] = pd.to_timedelta(df["duration"])
        duration_stats = df.groupby(pd.qcut(df["duration"], 5))["profit"].agg(
            ["mean", "count"]
        )
    ...
    def _generate_trade_analysis(self, output_dir: Path):
        ...
        df["drawdown"] = (
            df["cumulative_profit"] - df["cumulative_profit"].expanding().max()
        )
    ...
    def _create_dashboard(self, output_dir: Path):
        ...
        # Add equity curve
        fig.add_trace(go.Scatter(y=self.equity_curve, name="Equity"), row=1, col=1)

        # Add drawdown
        rolling_max = self.equity_curve.expanding().max()
        drawdown = (self.equity_curve - rolling_max) / rolling_max
        fig.add_trace(go.Scatter(y=drawdown * 100, name="Drawdown %"), row=1, col=2)

        # Add profit distribution
        fig.add_trace(
            go.Histogram(x=df["profit"], name="Profit Distribution"), row=2, col=1
        )

        # Add position sizes
        fig.add_trace(go.Box(y=df["size"], name="Position Sizes"), row=2, col=2)

        # Add daily returns
        daily_returns = df.groupby(df["timestamp"].dt.date)["profit"].sum()
        fig.add_trace(go.Bar(y=daily_returns, name="Daily Returns"), row=3, col=1)
